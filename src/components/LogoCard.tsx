...
function buildWaUrl(logo: Logo, price: string): string {
  const msg = encodeURIComponent(
    `Hi! I'm interested in purchasing the *${logo.title}* logo (${price}).\n\nListing: ${logo.logoUrl}\n\nIs it still available?`
  )
...
export default function LogoCard({ logo, layout = 'grid' }: LogoCardProps) {
  ...
  // Buka listing LogoGround — tombol utama "Buy"
  const handleBuyOnLogoGround = (e: React.MouseEvent) => {
    e.stopPropagation()
    if (logo.logoUrl) window.open(logo.logoUrl, '_blank')
  }

  // Buka WA dengan pesan pre-filled
  const handleOrderWa = (e: React.MouseEvent) => {
    e.stopPropagation()
    window.open(buildWaUrl(logo, priceDisplay), '_blank')
  }
      ...
      <>
        <article className={'logo-row' + (isSpecial ? ' on-sale' : '')} onClick={handleCardClick}>
          ...
          <div className="row-body">
            ...
            <div className="row-side">
              ...
              <div className="row-actions">
                ...
                {/* Tombol WA — selalu visible di list mode */}
                <button className="btn-action btn-wa-row" onClick={handleOrderWa} title="Order via WhatsApp">
                ...
                {/* Tombol Buy on LogoGround */}
                {logo.logoUrl && (
                  <button className="btn-action btn-buy-row" onClick={handleBuyOnLogoGround} title="Buy on LogoGround">
          ...
          <style jsx>{`
            ...
            .btn-wa-row:hover { background: rgba(37,211,102,0.25); }
            .btn-buy-row {
              background: rgba(245,200,66,0.9);
              color: #0A0A0F;
            }
            .btn-buy-row:hover { background: rgba(245,200,66,1); }

            @media (max-width: 560px) {
              .logo-row { flex-wrap: wrap; }
              .row-image-wrap { width: 84px; height: 84px; }
              .row-body { flex-direction: column; gap: 10px; }
              .row-side { flex-direction: row; align-items: center; width: 100%; min-width: unset; }
  ...
  return (
    <article className={'logo-card' + (isSpecial ? ' on-sale' : '')} onClick={handleCardClick}>
      <div className="card-image-wrap">
        ...
        {/* Hover actions — pojok kanan atas */}
      ...
      <div className="card-body">
        ...
        {/* CTA row — selalu visible */}
        <div className="card-cta-row" onClick={e => e.stopPropagation()}>
          ...
          {logo.logoUrl && (
            <button className="cta-buy" onClick={handleBuyOnLogoGround} title="Buy on LogoGround">
              <svg viewBox="0 0 16 16" fill="none" width="13" height="13">
                <path d="M6 3H3a1 1 0 00-1 1v9a1 1 0 001 1h9a1 1 0 001-1v-3M10 2h4m0 0v4m0-4L7 9" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
              LogoGround
      ...
      <style jsx>{`
        ...
        .logo-card:hover .card-image { transform: scale(1.04); }
        ...
        .like-btn:hover { transform: scale(1.1); color: #FF4D6D; }
        .card-actions {
          position: absolute;
          top: 10px; right: 10px;
          display: flex;
        ...
        /* CTA row — selalu visible di bawah card */
        .card-cta-row {
          display: flex;
          gap: 8px;
          padding-top: 4px;
        ...
        .cta-wa:hover { background: rgba(37,211,102,0.25); }
        .cta-buy {
          display: flex;
          align-items: center;
          justify-content: center;
        ...
        .cta-buy:hover { background: rgba(245,200,66,1); }
      `}</style>
    </article>
  )
}
